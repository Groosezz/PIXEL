...
import submitit
import transformers
from datasets import Array3D, Features, Sequence, Value, load_dataset, load_metric,load_from_disk
...
from pixel import (
    ...
    BARTEncoderForQuestionAnswering,
    get_attention_mask,
    get_transforms,
...
def main():
    ...
    metric = load_metric("../cache/metrics/squad", cache_dir=training_args.output_dir)
    ...
    if model_args.encoder_only: 
        model = BARTEncoderForQuestionAnswering.from_pretrained(
            model_args.model_name_or_path,
            config=config ) 
    else:
        ...
        if modality == Modality.IMAGE:
            with training_args.main_process_first(desc="validation dataset map pre-processing"):
                if not model.config.isBinary:
                        ...
                        {
                            "pixel_values": Array3D(
                                dtype="float32", shape=(1, processor.pixels_per_patch, processor.max_pixels_len)
                            ),
                            "attention_mask": Sequence(Value(dtype="int32")),
                            "offset_mapping": Sequence(Sequence(Value(dtype="int32"))),
                            "example_id": Value(dtype="string", id=None),
