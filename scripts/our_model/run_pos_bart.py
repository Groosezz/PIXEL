...
class ModelArguments:
    ...
    encoder_only: bool = field(
        default=False,
        metadata={ "help": "whether or not to just use the PIXEL encoder hidden states to do finetuning"
        },
    )
...
def main():
    ...
    if model_args.encoder_only: 
        ...
        model = BARTForTokenClassification.from_pretrained(
            model_args.model_name_or_path,
            config=config)
    ...
    if modality == Modality.IMAGE:
        ...
        new_ppr = math.sqrt(data_args.max_seq_length)
        
        if model_args.encoder_only:
            resize_model_embeddings(model, processor.max_seq_length)
        else:
            old_pos_embeds = model.model.vit.embeddings.position_embeddings[:, : data_args.max_seq_length + 1, :]
    ...
    train_dataset = get_dataset(config, data_args, processor, modality, Split.TRAIN, config.isBinary) if training_args.do_train else None
    eval_dataset = get_dataset(config, data_args, processor, modality, Split.DEV, config.isBinary) if training_args.do_eval else None
    test_dataset = get_dataset(config, data_args, processor, modality, Split.TEST, config.isBinary) if training_args.do_predict else None
