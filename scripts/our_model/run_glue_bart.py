...
def main():
    ...
    if data_args.task_name is not None:
        ...
        raw_datasets = load_from_disk(data_args.data_cache_dir)
    ...
    if model_args.encoder_only: 
        model = BARTEncoderForSequenceClassification.from_pretrained(
            model_args.model_name_or_path,
            pooling_mode=model_args.pooling_mode,
            config=config ) 
    else:
    ...
    preprocess_fn = get_preprocess_fn(data_args, processor, modality, (sentence1_key, sentence2_key), config.isBinary)
    ...
    if data_args.task_name is not None:
        """
        metric = load_metric("../cache/metrics/glue",
                             config_name = data_args.task_name,
                             cache_dir = f"../cache/metrics/glue/{data_args.task_name}")
        """
    ...
    def compute_metrics(p: EvalPrediction):
        ...
        if data_args.task_name is not None:
            ...
            #result = metric(preds, p.label_ids)
            #if len(result) > 1:
