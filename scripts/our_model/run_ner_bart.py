...
def main():
    ...
    train_dataset = (
        get_dataset(config, data_args, processor, modality, Split.TRAIN, labels,config.isBinary) if training_args.do_train else None
    )
    eval_dataset = (
        get_dataset(config, data_args, processor, modality, Split.DEV, labels,config.isBinary) if training_args.do_eval else None
    )
    test_dataset = (
        get_dataset(config, data_args, processor, modality, Split.TEST, labels,config.isBinary) if training_args.do_predict else None
    )
