...
from GPUtil import showUtilization as GPUstatus
...
def main(config_dict: Dict[str, Any] = None):
    ...
    """ no validation set temporary
    validation_dataset = load_dataset(
        data_args.validation_dataset_name, split=data_args.validation_split, use_auth_token=model_args.use_auth_token
    )
    """
    
    # use a small fraction of train_dataset to be validation set
    train_dataset,validation_dataset = train_dataset.train_test_split(test_size=0.0001).values()
    logger.info(f"Train Test split finished, the sizes of datasets are: train_datasets-{len(train_dataset)}, validation_dataset-{len(validation_dataset)}")
    ...
    # Also reinitnalize the prediction layer, set output size from 16*16*3 to 8*8*3
    model.decoder.decoder_pred = torch.nn.Linear(
    ...
    for param in  model.decoder.decoder_pred.parameters():
        param.requires_grad = True
