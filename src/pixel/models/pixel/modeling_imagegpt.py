...
from transformers.modeling_utils import PreTrainedModel,Conv1D, find_pruneable_heads_and_indices, prune_conv1d_layer
...
class ImageGPTPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, (nn.Linear, Conv1D, nn.Conv2d)):
...
class ImageGPTModel(ImageGPTPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # config adjustment
        self.h = nn.ModuleList([ImageGPTBlock(config, layer_idx=i) for i in range(config.decoder_num_hidden_layers)])
...
class ImageGPTForConditionalGeneration(ImageGPTPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        # for identical input and output in Autoregressive
        # we need the set the shape of input ids as [BSZ, Sequence length, vocab_size]
        # where vocab_size = patch**2*n_channels
        # convert input_ids to imgs [BSZ, n_channels, height, weight]
        # pixel_values = self.unpatchify(input_ids)
        # Ignore this
        ...
        # set masking ratio to 0 will get shape [BSZ, sequence len +1 (cls token), hidden size] 
        latent = vit_outputs.last_hidden_state
        
        """
        ### ignore this, we do not use patch mask
        # ids to restore the original input from masked
        ids_restore = vit_outputs.ids_restore
        # patch mask
        mask = vit_outputs.mask

        
        # append mask tokens to latent
        mask_token = nn.Parameter(torch.zeros(1, 1, self.config.hidden_size))
        ...
        latent = torch.cat([latent[:, :1, :], x_], dim=1)  # append cls token
        
        
        # merge span mask and padding mask for cross attention use
        # here for span mask, we need to set mask == 0
        merged_mask = torch.bitwise_and(mask == 1, attention_mask == 1).long()
        """
        batch_size, _ = attention_mask.shape
        encoder_attention_mask = torch.cat((torch.ones((batch_size, 1), device=attention_mask.device), attention_mask), dim=1)
        
        
        
        # convert pixel_values into patch_embeddings
        decoder_inputs_embeds= self.vit.embeddings.patch_embeddings(pixel_values) # [N, L, hidden_size]
        
        # add one start patch
        batch_size, _, _ = decoder_inputs_embeds.shape
        sos_token = nn.Parameter(torch.zeros(1, 1, self.config.hidden_size))
        sos_tokens = sos_token.expand(decoder_inputs_embeds.shape[0], -1, -1)
        decoder_inputs_embeds = torch.concat((sos_tokens, decoder_inputs_embeds), dim = 1)
        decoder_attention_mask = torch.cat((torch.ones((batch_size, 1), device=attention_mask.device), attention_mask), dim=1)
        
        
        # decoder_input_ids should be the shape [N, L, P*P*3]
        outputs = self.gpt(
        ...
        '''
        loss = self.forward_loss(pixel_values, shift_logits, attention_mask)
        # change to merged mask for only focusing on the loss of masked patches
        ...
        # from hidden dim to imgs
        output_logits = self.unpatchify(shift_logits)
        ...
        return Seq2SeqLMOutput(
            ...
            encoder_last_hidden_state=latent,
            encoder_hidden_states=vit_outputs.hidden_states,
            encoder_attentions=vit_outputs.attentions,
        )
